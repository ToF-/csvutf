...
instance FromNamedRecord U where parseNamedRecord v = U <$> v .: "Donnée"
...
spec = do
    describe "decoding utf8 csv data" $ do
        it "should process unicode in field data" $ do
            BS.writeFile testStdFile (TE.encodeUtf8 (T.unlines ["Donnee" ,"42€"]))
        ...
        it "should process unicode including in field names" $ do
            let writeContent = TE.encodeUtf8 (T.unlines ["Donnée" ,"42€"])
            ...
            result `shouldBe`   Right (fromList ["Donnée"],fromList [U {u = "42€"}])
